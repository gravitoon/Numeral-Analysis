...
#
# If sgn f(p_n-1) sgn f(p_n) < 0, then p_n is the x-intercept of the line joining (p_n-1, f(p_n-1))
# and (p_n,f(p_n)). If not choose p_n as the x-intercept of the line joining (p_n-2, f(p_n-2)) and
# (p_n,f(p_n)) and then interchange the indices on p_n-2 and p_n-1
#
# Written by Jose Luis Paredes 08/27/2023
...
def false_position_method(initial_approx, tolerance = 0.001, max_iter = 20):
    ...
    while max_iter >= iter:
        ...
        print("{0:<3d}   {1:^.9f}".format(iter, p_n))
        ...
        # Interchange p_n-2, f(p_n-2) with  p_n-1, f(p_n-1)
        if q * q_1 < 0:
            p_0 = p_1
            q_0 = q_1
...
if __name__ == '__main__':
    false_position_method([0.5, np.pi/4], 0.0000001, 15)
